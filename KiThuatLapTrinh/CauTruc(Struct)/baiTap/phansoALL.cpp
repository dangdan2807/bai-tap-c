...
void Menu()
{
	...
	printf("\n2. Xuat phan so ");
	printf("\n3. Rut gon phan so ");
	printf("\n4. Tong, Hieu, Tich, Thuong hai phan so ");
	...
	do 
	{
		...
		switch (chon)
		{
			...
			case 4 : printf ("Nhap phan so 1 : ");InputPS(ps1);
					 ...
					 ps = TongPS(ps1, ps2); 
					 printf ("Phan so tong ps1+ps2 = "); OutputPS(ps);
					 ps = HieuPS(ps1, ps2); 
					 printf ("\nPhan so hieu ps1-ps2 = "); OutputPS(ps);
					 ps = TichPS(ps1, ps2); 
					 printf ("\nPhan so tich ps1*ps2 = "); OutputPS(ps);
					 ps = ThuongPS(ps1, ps2); 
					 printf ("\nPhan so thuong ps1/ps2 = "); OutputPS(ps); break ;
		}
	}while (chon != 0);	
}
